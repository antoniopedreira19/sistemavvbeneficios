...
import { Building2, Wallet, DollarSign, Users, PieChart as PieIcon, BarChart3, Filter, TrendingUp, Trophy, CheckCircle2, AlertCircle // Ícone para pendentes
} from "lucide-react";
import { ComposedChart, Line, Bar, BarChart, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, Legend } from "recharts";
...
export default function Dashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data
      } = await supabase.from("lotes_mensais").select("competencia").order("created_at", {
        ascending: false
      });
      const unique = Array.from(new Set(data?.map(d => d.competencia))).filter(Boolean);
      if (unique.length === 0) {
        const hoje = new Date();
        const mesAtual = format(hoje, "MMMM/yyyy", {
          locale: ptBR
        });
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        count: totalVidasAtivas
      } = await supabase.from("colaboradores").select("*", {
        count: "exact",
        head: true
      }).eq("status", "ativo");
      ...
      const {
        data: lotesMes
      } = await supabase.from("lotes_mensais").select(`
          id, 
          empresa_id, 
          status, 
          total_colaboradores, 
      ...
      if (lotesIds.length > 0) {
        const {
          data
        } = await supabase.from("colaboradores_lote").select("sexo, salario").in("lote_id", lotesIds);
      ...
      const {
        data: notasMes
      } = await supabase.from("notas_fiscais").select("valor_total").eq("competencia", selectedCompetencia).eq("nf_emitida", true);

      // F. Histórico
      const {
        data: historicoLotes
      } = await supabase.from("lotes_mensais").select("competencia, total_colaboradores, valor_total, created_at").order("created_at", {
        ascending: true
      });
      ...
      const {
        data: allActiveCompanies
      } = await supabase.from("empresas").select("id, nome, colaboradores(count)").eq("status", "ativa").eq("colaboradores.status", "ativo");
      return {
  ...
  // Gráfico Gênero
  const genderStats = dashboardData?.colaboradores.reduce((acc: any, curr) => {
  ...
  const genderChartData = [{
    name: "Masculino",
    value: genderStats["M"] || 0
  }, {
    name: "Feminino",
    value: genderStats["F"] || 0
  }].filter(d => d.value > 0);
  ...
  const top5Revenue = Object.entries(revenueByCompany).map(([name, value]) => ({
    name,
    value
  ...
  const companiesSentData = dashboardData?.lotesMes.map((lote: any) => ({
  ...
  const companiesPendingData = dashboardData?.allActiveCompanies.filter((empresa: any) => !companiesSentIds.has(empresa.id)) // Remove quem já enviou
  .map((empresa: any) => {
  ...
  return <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-white p-2 rounded-lg border shadow-sm">
          ...
          <Select value={selectedCompetencia} onValueChange={setSelectedCompetencia}>
            ...
            <SelectContent>
              {competencias.map((comp: string) => <SelectItem key={comp} value={comp}>
                  {comp}
                </SelectItem>)}
            </SelectContent>
          </Select>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        <KpiCard title="Empresas Ativas" value={dashboardData?.empresasAtivas} icon={Building2} description="Total na carteira" />

        {/* CARD CLICÁVEL COM NOVA LÓGICA NO MODAL */}
        <KpiCard title="Empresas no Mês" value={totalEmpresasNoMes} icon={CheckCircle2} description={`Enviaram em ${selectedCompetencia.split("/")[0]}`} iconColor="text-purple-600" className="cursor-pointer hover:shadow-md transition-shadow active:scale-95" onClick={() => setShowCompaniesModal(true)} />

        <KpiCard title="Vidas no Mês" value={totalVidasMes} icon={Users} description="Total processado" iconColor="text-orange-600" />

        <KpiCard title="Faturamento Esperado" value={`R$ ${dashboardData?.faturamentoEsperado.toLocaleString("pt-BR", {
        minimumFractionDigits: 0
      })}`} icon={Wallet} description="Potencial (Vidas Totais)" iconColor="text-blue-600" />

        <KpiCard title="Faturamento Real" value={`R$ ${faturamentoRealizado.toLocaleString("pt-BR", {
        minimumFractionDigits: 0
      })}`} icon={DollarSign} description="Notas Emitidas" iconColor="text-green-600" highlight />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-7">
          ...
          <CardContent className="pl-0">
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={evolutionChartData} margin={{
                top: 20,
                right: 30,
                left: 20,
                bottom: 5
                  ...
                  <YAxis yAxisId="left" fontSize={12} tickLine={false} axisLine={false} tickFormatter={val => `R$${val / 1000}k`} />
                  ...
                  if (name === "Faturamento") return [value.toLocaleString("pt-BR", {
                    style: "currency",
                    currency: "BRL"
                  }), name];
                  return [value, name];
                  ...
                  <Legend />
                  <Bar yAxisId="left" dataKey="faturamento" name="Faturamento" fill="#82ca9d" barSize={30} radius={[4, 4, 0, 0]} />
                  <Line yAxisId="right" type="monotone" dataKey="vidas" name="Vidas" stroke="#8884d8" strokeWidth={3} dot={{
                  r: 4
                }} />
                </ComposedChart>
              </ResponsiveContainer>
        ...
        <Card className="col-span-3">
          ...
          <CardContent>
            <div className="h-[300px] w-full">
              {top5Revenue.length > 0 ? <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={top5Revenue} layout="vertical" margin={{
                top: 5,
                right: 30,
                left: 20,
                bottom: 5
                    ...
                    <YAxis dataKey="name" type="category" width={100} fontSize={11} tickLine={false} axisLine={false} tickFormatter={val => val.length > 15 ? val.slice(0, 15) + "..." : val} />
                    <Tooltip cursor={{
                  fill: "transparent"
                }} formatter={(value: number) => [`R$ ${value.toLocaleString("pt-BR")}`, "Faturamento"]} />
                    <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={25}>
                      {top5Revenue.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS_TOP5[index % COLORS_TOP5.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer> : <div className="h-full flex items-center justify-center text-muted-foreground text-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <div className="h-[300px] w-full flex items-center justify-center">
              {genderChartData.length > 0 ? <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={genderChartDataWithPct} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value" label={({
                  name,
                  pct
                }) => `${name} (${pct})`}>
                      {genderChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS_GENDER[index % COLORS_GENDER.length]} />)}
                    </Pie>
                    <Tooltip formatter={(value: number) => [value, "Colaboradores"]} />
                    <Legend />
                  </PieChart>
                </ResponsiveContainer> : <div className="text-muted-foreground text-sm">Sem dados.</div>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="h-[300px] w-full">
              {totalVidasMes > 0 ? <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={salaryChartData} layout="vertical" margin={{
                top: 5,
                right: 30,
                left: 40,
                bottom: 5
                    ...
                    <Tooltip cursor={{
                  fill: "transparent"
                }} formatter={(value: number, name: string, props: any) => [`${value} pessoas (${props.payload.pct})`, "Quantidade"]} />
                    <Bar dataKey="quantidade" fill="#8884d8" radius={[0, 4, 4, 0]} barSize={25}>
                      {salaryChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS_SALARY[index % COLORS_SALARY.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer> : <div className="h-full flex items-center justify-center text-muted-foreground text-sm">Sem dados.</div>}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <CompaniesInMonthModal open={showCompaniesModal} onOpenChange={setShowCompaniesModal} sentData={companiesSentData} pendingData={companiesPendingData} competencia={selectedCompetencia} />
    </div>;
}

// --- NOVO COMPONENTE DO MODAL COM ABAS ---
function CompaniesInMonthModal({
  open,
  onOpenChange,
  sentData,
  pendingData,
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] flex flex-col">
        ...
        <Tabs defaultValue="enviados" className="flex-1 overflow-hidden flex flex-col">
          <TabsList className="grid w-full grid-cols-2 mb-2">
            <TabsTrigger value="enviados" className="data-[state=active]:bg-green-100 data-[state=active]:text-green-800">
          ...
          <TabsContent value="enviados" className="flex-1 overflow-y-auto border rounded-md p-0 m-0">
            <Table>
              ...
              <TableBody>
                {sentData.length > 0 ? sentData.map((item: any, idx: number) => <TableRow key={idx}>
          ...
          <TabsContent value="pendentes" className="flex-1 overflow-y-auto border rounded-md p-0 m-0">
            <Table>
              ...
              <TableBody>
                {pendingData.length > 0 ? pendingData.map((item: any, idx: number) => <TableRow key={idx}>
                      <TableCell className="font-medium text-red-700">{item.empresa}</TableCell>
                      <TableCell className="text-center">
                        {item.vidas > 0 ? <span className="font-bold">{item.vidas}</span> : <Badge variant="outline" className="text-muted-foreground">
                            Sem dados
                          </Badge>}
                      </TableCell>
                      <TableCell className="text-right text-muted-foreground">
                        {item.previsao_faturamento.toLocaleString("pt-BR", {
                    style: "currency",
                    currency: "BRL"
                  })}
                      </TableCell>
...
function KpiCard({
  ...
  return <Card className={cn(highlight ? "border-green-500 bg-green-50" : "", className)} onClick={onClick}>
