-- Remove todas as triggers relacionadas primeiro
DROP TRIGGER IF EXISTS trigger_atualizar_totais_lote ON lotes_mensais;
DROP TRIGGER IF EXISTS trigger_atualizar_totais_ao_concluir_lote ON lotes_mensais;
DROP TRIGGER IF EXISTS trigger_criar_apolice_ao_concluir_lote ON lotes_mensais;
DROP TRIGGER IF EXISTS trigger_criar_nota_fiscal_empresa_ativa ON lotes_mensais;

-- Remove as funções antigas
DROP FUNCTION IF EXISTS public.atualizar_totais_ao_concluir_lote() CASCADE;
DROP FUNCTION IF EXISTS public.criar_apolice_ao_concluir_lote() CASCADE;
DROP FUNCTION IF EXISTS public.criar_nota_fiscal_empresa_ativa() CASCADE;

-- Cria nova função consolidada que calcula totais e cria apólice/NF
...
BEGIN
  ...
  IF NEW.status = 'concluido' AND (OLD.status IS NULL OR OLD.status != 'concluido') THEN
    ...
    -- Cria apólice se empresa está em implementação
...
-- Cria a trigger BEFORE UPDATE
CREATE TRIGGER trigger_atualizar_totais_ao_concluir_lote
  BEFORE UPDATE ON lotes_mensais
  FOR EACH ROW
  EXECUTE FUNCTION public.atualizar_totais_ao_concluir_lote();